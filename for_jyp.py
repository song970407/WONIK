...
def main(state_order, action_order, model_filename):
    ...
    m = get_multi_linear_residual_fast_model(state_dim, action_dim, state_order, action_order).to(device)
    m.load_state_dict(torch.load(model_filename, map_location=device))
    m.eval()
    norms_A = []
    norms_B = []
    for i in range(state_order):
        norms_A.append(torch.norm(m.A[i]).item())
    for j in range(action_order):
        norms_B.append(torch.norm(m.B[j]).item())
    ...
    return ys, predicted_ys, us, norms_A, norms_B
...
if __name__ == '__main__':
    ...
    for i in range(len(model_filenames)):
        ys, predicted_ys, us, norms_A, norms_B = main(state_orders[i], action_orders[i], model_filenames[i])
        yss.append(ys)
        predicted_yss.append(predicted_ys)
        uss.append(us)
        norms_As.append(norms_A)
        norms_Bs.append(norms_B)
    plt.figure(123)
    ...
    for i in range(len(predicted_yss)):
        plt.figure(i)
        plt.title('Predicted Glass TC, State Order: ' + str(state_orders[i]) + ' Action Order: ' + str(action_orders[i]))
        plt.plot(predicted_yss[i][0])
        plt.ylim([100, 450])
        plt.show()

    plt.figure(12345)
    plt.title('Real Work Set')
    plt.plot(uss[0][0])
    plt.ylim([100, 450])
    plt.show()

    plt.figure(12345)
    for i in range(len(norms_As)):
        plt.plot(norms_As[i], label=str(i))
    plt.legend()
    plt.show()

    plt.figure(1234)
    for j in range(len(norms_Bs)):
        plt.plot(norms_Bs[j], label=str(j))
    plt.legend()
    plt.show()
