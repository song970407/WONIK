...
class ReparamMultiLinearSSM(nn.Module):
    def __init__(self,
        ...
        self.state_order = state_order
        self.action_order = action_order
        self.A_list = nn.ModuleList([ReparameterizedLinear(state_dim, state_dim, reparam_method='Expo') for _ in range(state_order)])
        self.B_list = nn.ModuleList([ReparameterizedLinear(action_dim, state_dim, reparam_method='Expo') for _ in range(action_order)])
