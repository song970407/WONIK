...
class MultiLinearResSSM(nn.Module):
    def __init__(self,
                 state_dim: int,
                 action_dim: int,
                 state_order: int,
                 action_order: int):
        super(MultiLinearResSSM, self).__init__()
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.state_order = state_order
        self.action_order = action_order
        self.A_list = nn.ModuleList([nn.Linear(state_dim, state_dim, bias=False) for _ in range(state_order)])
        self.B_list = nn.ModuleList([nn.Linear(action_dim, state_dim, bias=False) for _ in range(action_order)])
        # In Multi-step Linear model, we initialize as zeros
        for l in self.A_list:
            l.weight.data = torch.zeros_like(l.weight.data)
        for l in self.B_list:
            l.weight.data = torch.zeros_like(l.weight.data)
    ...
    def forward(self, x, u):
        ...
        :return:
        """
        ar_terms = torch.einsum('bpx, txy -> bpy')
...
class MultiLinearResFastSSM(nn.Module):
    def __init__(self,
                 state_dim: int,
                 action_dim: int,
                 state_order: int,
                 action_order: int):
        super(MultiLinearResFastSSM, self).__init__()
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.state_order = state_order
        self.action_order = action_order
