...
def load_preco_data(paths: Union[str, List[str]],
                    scaling: bool = True,
                    state_scaler: tuple = None,
                    action_scaler: tuple = None,
                    action_ws: bool = True,
                    preprocess: bool = False,
                    receding_history: int = 1,
                    device: str = 'cpu'):
    if isinstance(paths, str):
        paths = [paths]
    states = []
    ...
    for path in paths:
        ...
        if preprocess:
            ...
            for i in range(step_df_length):
                if step_df[i][0] == 'SLOW_COOL':
                    state = state[first - receding_history + 1:i + 1, :]
                    action = action[first - receding_history + 1:i, :]
                    break
        states.append(state)
        actions.append(action)
    s_min = 10000000000
    ...
    if scaling:
        if state_scaler is None:
            for state in states:
                s_min = min(state.min(), s_min)
                s_max = max(state.max(), s_max)
        else:
            s_min = state_scaler[0]
            s_max = state_scaler[1]
        if action_scaler is None:
            for action in actions:
                a_min = min(action.min(), a_min)
                a_max = max(action.max(), a_max)
        else:
            a_min = action_scaler[0]
            a_max = action_scaler[1]
        for idx in range(len(states)):
            states[idx] = (states[idx] - s_min) / (s_max - s_min)
            actions[idx] = (actions[idx] - a_min) / (a_max - a_min)
        info['state_min'] = s_min
        info['state_max'] = s_max
        info['action_min'] = a_min
        info['action_max'] = a_max
    return states, actions, info
