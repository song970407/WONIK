...
def main():
    ...
    runner = Runner(m=m, optimizer_mode=optimizer_mode, state_scaler=scaler, action_scaler=scaler, alpha=alpha, timeout=time_limit)
    ...
    for t in range(T - H):
        ...
        start = time.time()
        workset = runner.solve(history_tc, history_ws,
