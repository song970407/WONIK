...
train_data_path = train_data_path = ['docs/new_data/grnn/data_3.csv', 'docs/new_data/icgrnn/data_3.csv', 'docs/new_data/expert/data_3.csv', 'docs/new_data/overshoot/data_2.csv']
...
opt = torch.optim.Adam(m.parameters(), lr=0.000005)
scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=10)
iters = len(train_loader)
EPOCHS = 10
TEST_EVERY = 100
run = wandb.init(entity='sentinel',
                 name='State Order ' + str(state_order) + ' Action Order ' + str(action_order) + ' H ' + str(H) + ' Additional',
                 reinit=True,
