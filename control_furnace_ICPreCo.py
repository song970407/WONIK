...
class Runner:
    ...
    def solve(self, history_tc, history_ws, target, weight=None):
        ...
        action = self.solver.solve_mpc(self.g, (history_tc, history_ws), target, weight)
        return action()[:, 0:1, :]
...
def main():
    ...
    weight = torch.stack([torch.ones_like(target), torch.ones_like(target)], dim=-1).to(
        device)  # [180 x 908 x 2], 0: when negative, 1: when positive
    ...
    for t in range(T - H):
        ...
        workset = runner.solve(history_tc, history_ws, target[:, t:t + H],
                               weight[:, t:t + H])  # [40 x 1] torch.Tensor, use this workset to the furnace
        end = time.time()
